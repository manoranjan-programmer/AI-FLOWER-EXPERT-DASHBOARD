...
import { Activity, PieChart as PieIcon, BarChart2, Sun, Droplets, Zap, Layers, ThumbsUp, Monitor, Cpu } from 'lucide-react';
...
export default function AnalyticsCharts({ chartsData = {} }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Chart 1: Search Volume, Chatbot Latency & Activity Trend */}
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h3 className="text-base font-bold text-gray-900 flex items-center gap-2">
              <Activity className="w-5 h-5 text-blue-600" />
              Search Volume & Processing Latency Trends
            </h3>
            <p className="text-xs text-gray-500 mt-0.5">
              Live chatbot performance, prediction volume, and latency over time.
            </p>
          </div>

          <div className="flex items-center gap-1 p-1 rounded-xl bg-gray-100 border border-gray-200 text-xs font-semibold">
            {['uploads', 'chats', 'predictions', 'users', 'generationTimeMs', 'classificationTimeMs'].map(m => (
              <button
                key={m}
                onClick={() => setTimelineMetric(m)}
                className={`px-3 py-1.5 rounded-lg capitalize font-medium transition-all ${timelineMetric === m
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 space-y-4">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              Top identified flowers ranked by frequency in MongoDB.
            </p>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 space-y-4">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              Breakdown of identified species by botanical family.
            </p>
          </div>
      ...
      {/* Row 3: Classifier Confidence Distribution & User Feedback / Device Breakdown */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 space-y-4">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              Accuracy distribution across predictions.
            </p>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 space-y-4">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              Ratio of positive vs negative responses.
            </p>
          </div>
              ...
              <PieChart>
                ...
                >
                  {feedbackDistribution.map((entry, index) => (
                    <Cell key={`cell-fb-${index}`} fill={entry.color || COLORS[index % COLORS.length]} />
                  ))}
                </Pie>
                <Tooltip contentStyle={{ backgroundColor: '#ffffff', borderColor: '#e5e7eb', borderRadius: '12px' }} />
                <Legend layout="horizontal" verticalAlign="bottom" align="center" wrapperStyle={{ fontSize: '10px' }} />
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 space-y-4">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              Client platform analytics from User_Activity.
            </p>
          </div>
